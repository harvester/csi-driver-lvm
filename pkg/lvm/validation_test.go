...
func TestValidateVGName(t *testing.T) {
	tests := []struct {
		name    string
		vgName  string
		wantErr bool
	}{
		{name: "simple", vgName: "vg"},
		{name: "all valid characters", vgName: ".vg-1_test+data"},
		{name: "empty", wantErr: true},
		{name: "leading hyphen", vgName: "--config", wantErr: true},
		{name: "current directory", vgName: ".", wantErr: true},
		{name: "parent directory", vgName: "..", wantErr: true},
		{name: "path separator", vgName: "vg/name", wantErr: true},
		{name: "tag selector", vgName: "@storage", wantErr: true},
		{name: "whitespace", vgName: "vg name", wantErr: true},
		{name: "control character", vgName: "vg\n--config", wantErr: true},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := validateVGName(tt.vgName)
			if tt.wantErr && err == nil {
				t.Fatalf("expected %q to be rejected", tt.vgName)
			}
			if !tt.wantErr && err != nil {
				t.Fatalf("expected %q to be accepted: %v", tt.vgName, err)
			}
		})
	}
}
...
func TestParseLVMParameters(t *testing.T) {
	tests := []struct {
		...
		{
			name:       "option-like vg",
			parameters: map[string]string{"type": DmThinType, "vgName": "--config"},
			wantErr:    true,
		},
		{
			name:       "path-like vg",
			parameters: map[string]string{"type": DmThinType, "vgName": "vg/name"},
			wantErr:    true,
		},
	}
...
func TestValidateNodePublishRequest(t *testing.T) {
	...
	t.Run("invalid volume group", func(t *testing.T) {
		req := validRequest()
		req.VolumeContext["vgName"] = "--config"
		if _, err := validateNodePublishRequest(req); err == nil {
			t.Fatal("expected an option-like volume group to fail")
		}
	})
...
func TestMetadataFromPersistentVolume(t *testing.T) {
	...
	invalidVG := validPersistentVolume("invalid-vg")
	invalidVG.Spec.CSI.VolumeAttributes["vgName"] = "--config"
	if _, _, _, err := metadataFromPV(invalidVG); err == nil {
		t.Fatal("expected an option-like volume group to fail")
	}
}
