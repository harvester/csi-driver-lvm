...
var validVGNamePattern = regexp.MustCompile(`^[A-Za-z0-9+_.-]+$`)
...
func validateVGName(name string) error {
	...
	if name == "." || name == ".." {
		return fmt.Errorf("volume group name %q is reserved", name)
	}
	if name[0] == '-' {
		return fmt.Errorf("volume group name %q must not begin with a hyphen", name)
	}
	if !validVGNamePattern.MatchString(name) {
		return fmt.Errorf("volume group name %q contains invalid characters", name)
	}
	return nil
}
...
func parseLVMParameters(parameters map[string]string) (string, string, error) {
	...
	if lvmType != StripedType && lvmType != DmThinType {
		return "", "", fmt.Errorf("lvmType is incorrect: %q", lvmType)
	}
...
func nodeFromAccessibility(requirement *csi.TopologyRequirement) (string, error) {
	...
	for _, topology := range requirement.GetPreferred() {
		...
		if len(requisiteNodes) > 0 && !allowed {
			return "", fmt.Errorf("preferred node %q is not present in requisite topologies", node)
		}
		return node, nil
	}

	if len(requisiteNodes) == 0 {
		return "", fmt.Errorf("accessibility requirements do not contain %q", topologyKeyNode)
	}
...
func lvmAttributesFromPV(name string, source *v1.CSIPersistentVolumeSource) (string, string, error) {
	if source == nil {
		return "", "", fmt.Errorf("persistent volume %q has no CSI source", name)
	}

	vgName := source.VolumeAttributes["vgName"]
	if vgName == "" {
		return "", "", fmt.Errorf("persistent volume %q has no vgName attribute", name)
	}
	if err := validateVGName(vgName); err != nil {
		return "", "", fmt.Errorf("persistent volume %q has an invalid vgName attribute: %w", name, err)
	}
	lvmType := source.VolumeAttributes["type"]
	if lvmType != StripedType && lvmType != DmThinType {
		return "", "", fmt.Errorf("persistent volume %q has invalid LVM type %q", name, lvmType)
	}
...
func nodeFromPVAffinity(name string, affinity *v1.VolumeNodeAffinity) (string, error) {
	if affinity == nil || affinity.Required == nil {
		return "", fmt.Errorf("persistent volume %q has no required node affinity", name)
	}
	...
	for _, expression := range matchExpressionsFromTerms(affinity.Required.NodeSelectorTerms) {
		...
		if expression.Operator != v1.NodeSelectorOpIn {
			return "", fmt.Errorf(
				"persistent volume %q topology expression must use operator In",
				name,
			)
		}
	...
	if len(nodes) != 1 {
		return "", fmt.Errorf(
			"persistent volume %q must reference exactly one %q node, found %d",
			name,
			topologyKeyNode,
			len(nodes),
		)
	...
	return "", fmt.Errorf("unable to select node for persistent volume %q", name)
}
...
func requiredBytesFromPersistentVolume(volume *v1.PersistentVolume) (int64, error) {
	...
	if volume.Spec.CSI == nil {
		return 0, fmt.Errorf("persistent volume %q has no CSI source", volume.Name)
	}
	value := volume.Spec.CSI.VolumeAttributes["RequiredBytes"]
	size, err := strconv.ParseInt(value, 10, 64)
	if err != nil || size <= 0 {
		return 0, fmt.Errorf("persistent volume %q has invalid RequiredBytes attribute %q", volume.Name, value)
	}
	return size, nil
}
...
func metadataFromSnapshotContent(content *snapv1.VolumeSnapshotContent) (string, string, int64, error) {
	...
	if content.Spec.Source.VolumeHandle == nil || *content.Spec.Source.VolumeHandle == "" {
		return "", "", 0, fmt.Errorf("snapshot content %q has no source volume handle", content.Name)
	}
	if content.Status == nil ||
		...
		*content.Status.RestoreSize <= 0 {
		return "", "", 0, fmt.Errorf("snapshot content %q is not ready to restore", content.Name)
	}
...
func preProvisionedSnapshotMetadata(content *snapv1.VolumeSnapshotContent) (string, int64, error) {
	...
	if statusHandle != "" && sourceHandle != "" && statusHandle != sourceHandle {
		return "", 0, fmt.Errorf(
			"pre-provisioned snapshot content %q has conflicting snapshot handles %q and %q",
			content.Name,
			statusHandle,
			sourceHandle,
		)
	...
	if snapshotID == "" {
		return "", 0, fmt.Errorf("pre-provisioned snapshot content %q has no snapshot handle", content.Name)
	}
	...
	if *content.Status.RestoreSize < 0 {
		return "", 0, fmt.Errorf("pre-provisioned snapshot content %q has invalid restore size %d", content.Name, *content.Status.RestoreSize)
	}
